...
def get_drug_interactions(drug_name):
    ...
    if response.status_code == 200:
        ...
        drug_list = soup.find_all('ul', class_='interactions ddc-list-column-2')  # Find all 'ul' elements containing drug info
        if drug_list:
...
if __name__ == "__main__":
    ...
    quest_input = input("Would you like to know the interactions or info about the drug? (input info or interactions): ").lower()
    if quest_input == "info":
        info_input = input("Enter a drug name: ")
        get_drug_info(info_input)
    elif quest_input == "interactions":
        interact_input = input("Enter a drug name: ")
        get_drug_interactions(interact_input)
